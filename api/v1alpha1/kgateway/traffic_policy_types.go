...
type ResponseCompression struct {
	// Libraries lists the compression codecs to offer for responses, in preference order.
	// Envoy negotiates the codec to use based on the downstream request's `Accept-Encoding`
	// header: it picks the highest-quality codec the client accepts, and when the client
	// accepts multiple offered codecs with equal quality, the codec listed first here wins.
	// If the client accepts none of the offered codecs, the response is sent uncompressed.
	// Defaults to [Gzip].
	// +optional
	// +listType=atomic
	// +kubebuilder:validation:MinItems=1
	...
	// MinContentLengthBytes is the minimum response `Content-Length`, in bytes, required to trigger
	// compression. Responses below this size are sent uncompressed. Defaults to 30 when unset.
	// +optional
	// +kubebuilder:validation:Minimum=0
	...
	// compressible content types rather than adding to it. When unset, the defaults apply.
	// +optional
	// +listType=set
